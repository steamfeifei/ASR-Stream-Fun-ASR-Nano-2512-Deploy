# ASR-Stream-Fun-ASR-Nano-2512-Deploy
...
python wfunasr_wss_server.py
python web_client/serve_client.py
...
标点符号不太对。。
哎，你好。，你好。
ASR的识别率在没有大模型的情况下，是很重要。，但其实在A整的场景识别率差点
错几个字。，大模型依然知道你的意思。。所以流式识别的识别率差一点。，还是用可以用在产品上的。

```
我感觉没有预期的效果好


# 全文COPY：https://github.com/fengin/Fun-ASR-Nano-2512-Deploy 
...
- **推荐 (本地)**: 直接访问 `http://localhost:9002`。
  - 浏览器会自动允许麦克风权限。
  - 页面中 WebSocket 地址填入远程服务器 IP 即可 (例如 `ws://10.11.x.x:9002`)。
- **高级 (远程)**: 如果浏览器和 Web 服务不在同一台机器，需访问 `http://<Web_Server_IP>:9002`。
  - **注意**: Chrome 默认禁止非 HTTPS 网页使用麦克风。
  - **解决**: 需配置 `chrome://flags/#unsafely-treat-insecure-origin-as-secure` 才能使用麦克风。
